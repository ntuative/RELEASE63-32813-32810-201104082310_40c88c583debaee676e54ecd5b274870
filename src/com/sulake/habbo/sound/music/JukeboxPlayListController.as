package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      public function JukeboxPlayListController(param1:HabboSoundManagerFlash10, param2:HabboMusicController, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_715 = [];
         this.var_715.push(new NowPlayingMessageEvent(this.onNowPlayingMessage));
         this.var_715.push(new JukeboxSongDisksMessageEvent(this.onJukeboxSongDisksMessage));
         this.var_715.push(new JukeboxPlayListFullMessageEvent(this.onJukeboxPlayListFullMessage));
      ...
      {
         return HabboMusicPrioritiesEnum.const_180;
      }
      ...
      {
         this.var_67.stop(this.priority);
         this.var_440 = -1;
         this.var_1647 = -1;
         this.var_634 = false;
      }
      ...
      {
         if(param1 < 0 || param1 >= this.var_484.length)
         {
            return null;
         }
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         this.var_634 = _loc3_.currentSongId != -1;
         if(_loc3_.currentSongId >= 0)
         {
            this.var_67.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_180,_loc3_.syncCount / 1000,0,1,1);
            this.var_440 = _loc3_.currentSongId;
         }
         else
         {
            this.stopPlaying();
         ...
         {
            this.var_67.addSongInfoRequest(_loc3_.nextSongId);
         }
         this.var_1647 = _loc3_.currentPosition;
         this._soundManager.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_296,HabboMusicPrioritiesEnum.const_180,_loc3_.currentSongId,_loc3_.currentPosition));
      }
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            _loc7_ = this.var_67.getSongInfo(_loc5_) as SongDataEntry;
            if(_loc7_ == null)
            {
               _loc7_ = new SongDataEntry(_loc5_,-1,null,null,null);
               if(this.var_716.indexOf(_loc5_) < 0)
               {
                  this.var_716.push(_loc5_);
                  this.var_67.requestSongInfoWithoutSamples(_loc5_);
               }
            }
            _loc7_.diskId = _loc6_;
            this.var_484.push(_loc7_);
            _loc4_++;
         }
      ...
      private function onJukeboxPlayListFullMessage(param1:IMessageEvent) : void
      {
         ...
         this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_805));
      }
      ...
      private function onSongInfoReceivedEvent(param1:SongInfoReceivedEvent) : void
      {
         ...
         {
            _loc4_ = this.var_484[_loc2_];
            if(_loc4_.id == param1.id)
            {
               _loc5_ = _loc4_.diskId;
               _loc6_ = this.var_67.getSongInfo(param1.id) as SongDataEntry;
               if(_loc6_ != null)
               {
                  _loc6_.diskId = _loc5_;
                  this.var_484[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
         }
         var _loc3_:int = this.var_716.indexOf(param1.id);
         if(_loc3_ >= 0)
         {
            this.var_716.splice(_loc3_,1);
         }
         if(this.var_716.length == 0)
         {
            this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.PLAY_LIST_UPDATED));
