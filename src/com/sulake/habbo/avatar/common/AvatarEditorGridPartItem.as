package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private const const_2033:int = 13421772;
      
      private const const_2034:int = 16777215;
      
      private const const_348:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         this.var_701 = param3;
         this._window = param1;
         this.var_1336 = this._window.findChildByTag("BG_COLOR");
         this._colors = param4;
         this.var_1612 = param5;
         if(param3 == null)
         {
            this.var_698 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1614 = Math.max(this.var_1614,_loc6_.colorLayerIndex);
            }
         }
         this.var_403 = this.var_66.controller.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_698 != null && !this.var_1612)
            {
               _loc3_ = this.var_698;
            }
            else
            ...
            if(this.var_701)
            {
               switch(this.var_701.clubLevel)
               {
                  ...
                  case HabboClubLevelEnum.const_45:
                     _loc2_.style = HabboIconType.const_872;
                     break;
         ...
         if(this.isSelected)
         {
            this.var_1336.color = this.const_2033;
         }
         else
         {
            this.var_1336.color = this.const_2034;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_403.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_403.isFigureReady(_loc1_))
         {
            this.var_403.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_348[_loc6_] + "_" + FigureData.const_151;
               _loc4_ = this.var_403.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_348.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_348[_loc6_] + "_" + FigureData.const_151;
                  _loc4_ = this.var_403.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_578 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1613)
               {
                  ...
                  this.var_1613 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_578.width,this.var_578.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_578.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_578.y;
               if(this.var_1612 && _loc2_.colorLayerIndex > 0)
               {
